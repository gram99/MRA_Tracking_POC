...
def extract_mras_from_pdf(pdf_bytes, filename):
    ...
    extracted_findings = []
    # Set default start to 90 days before today to ensure positive window
    default_start = (datetime.now() - timedelta(days=90)).replace(tzinfo=None, hour=0, minute=0, second=0)
    ...
    for i, date_str in enumerate(date_matches if date_matches else ["Placeholder"]):
        try:
            ...
            deadline = (datetime.now() + timedelta(days=30)).replace(tzinfo=None)
...
def apply_sentinel_logic(df, auto_fix=False):
    ...
    today = datetime.now().replace(tzinfo=None, hour=0, minute=0, second=0)
    ...
    def process_row(row):
        ...
        if row['Status'] == "Closed": 
            ...
            if auto_fix:
                ...
                row['Risk_Status'] = "🟢 On Track (Fixed)"
            else:
...
if not st.session_state.mra_data.empty:
    ...
    with col1:
        ...
        st.metric("Critical/Overdue", crit_count, delta_color="inverse")
    ...
    with col2:
        ...
        fig_donut = px.pie(dist, values='count', names='Risk_Status', hole=0.5, 
                           color='Risk_Status', color_discrete_map={
                               "💀 OVERDUE": "#000000", "🚨 CRITICAL: 75%+ Elapsed": "#FF4B4B", 
                               "⚠️ WARNING: 50% Elapsed": "#FFAA00", "🟢 On Track": "#00CC96", "✅ Closed": "#2E7D32"})
        st.plotly_chart(fig_donut, use_container_width=True)

    with col3:
        heatmap_data = st.session_state.mra_data.groupby(['Owner', 'Risk_Status']).size().unstack(fill_value=0)
        fig_heat = px.bar(heatmap_data, barmode="stack", color_discrete_sequence=["#1B263B", "#415A77", "#778DA9"])
    ...
    with tab1:
        ...
        # --- THE FIX: FORCING POSITIVE DURATION FOR CHART ONLY ---
        def force_chart_dates(row):
            if row['Start_Date'] >= row['Deadline']:
                row['Start_Date'] = row['Deadline'] - timedelta(days=30)
            return row
        
        chart_df = chart_df.apply(force_chart_dates, axis=1)
